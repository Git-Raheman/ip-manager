...
export const SubnetScannerModal: React.FC<SubnetScannerModalProps> = ({
  ...
  const getDeviceIcon = (type: DeviceType) => {
    switch (type) {
      ...
      case 'router':
        return <Router className="w-3.5 h-3.5 text-emerald-400" />;
      case 'switch':
        return <Cpu className="w-3.5 h-3.5 text-cyan-400" />;
      case 'workstation':
        return <Monitor className="w-3.5 h-3.5 text-indigo-400" />;
      case 'printer':
        return <Printer className="w-3.5 h-3.5 text-amber-400" />;
      case 'firewall':
        return <Shield className="w-3.5 h-3.5 text-rose-400" />;
      case 'storage':
        return <HardDrive className="w-3.5 h-3.5 text-orange-400" />;
      default:
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-3 sm:p-5">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-4xl w-full max-h-[92vh] flex flex-col shadow-2xl overflow-hidden text-slate-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-[#171717] dark:text-white">Subnet Auto-Discovery Scanner</h3>
        ...
        <div className="shrink-0 px-6 py-3 bg-slate-950/90 border-b border-slate-800 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-3">
            {blocks.length > 1 && (
              <div className="flex items-center gap-1.5">
                ...
                <select
                  ...
                  className="bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1 text-slate-200 font-mono focus:outline-none focus:border-blue-500"
                >
                  {blocks.map((blk) => (
            ...
            <div className="text-slate-400">
              IP Range: <span className="text-slate-200 font-mono font-semibold">{targetIpsToScan[0] || '...'} – {targetIpsToScan[targetIpsToScan.length - 1] || '...'}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {scanStatus === 'scanning' ? (
              <button
                type="button"
                onClick={stopScan}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-950/80 hover:bg-rose-900 text-xs font-medium text-rose-300 transition-colors border border-rose-800"
              >
              ...
              <button
                type="button"
                onClick={startScan}
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-xs font-semibold text-white transition-colors shadow-sm"
              >
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-xs font-medium text-slate-300 transition-colors border border-slate-700"
            >
        ...
        <div className="shrink-0 px-6 py-4 bg-slate-950/70 border-b border-slate-800 space-y-3">
        ...
        {scanStatus === 'completed' && discoveredHosts.length === 0 && (
          <div className="mx-6 mt-3 p-3.5 rounded-xl bg-blue-950/40 border border-blue-800/60 text-blue-200 text-xs flex items-start gap-2.5">
            <Info className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <div className="font-semibold text-[#171717] dark:text-white">0 active physical hosts responded on {subnet.cidr}</div>
              <p className="text-slate-300 leading-relaxed">
                No ICMP replies or open TCP ports were detected across the targeted {targetIpsToScan.length} IP addresses. 
                Ensure this server has direct layer 2/3 network routing to <code className="text-blue-300 font-mono">{subnet.cidr}</code> and firewall rules allow ICMP/TCP probe packets.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="shrink-0 px-6 py-3 border-b border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-900/60">
            ...
            <div className="flex items-center gap-1 bg-slate-950 border border-slate-800 rounded-xl p-0.5 text-xs">
              <button
                ...
                className={`px-2.5 py-1 rounded-lg transition-colors ${
                  filterType === 'all' ? 'bg-[#171717] text-white dark:bg-slate-800 dark:text-white font-medium' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg transition-colors ${
                  filterType === 'existing'
                    ? 'bg-blue-100 text-blue-800 dark:bg-blue-950/80 dark:text-blue-300 font-medium'
                    : 'text-slate-400 hover:text-slate-200'
                }`}
              >
                In IPAM
                ...
                {filteredHosts.map((host) => {
                  return (
                    <tr
                      ...
                      className={`transition-colors cursor-pointer ${
                        host.isExistingInIPAM
                          ? 'bg-slate-950/40 text-slate-400 cursor-default'
                          : host.isSelected
                    ...
                    >
                      ...
                      <td className="py-2.5 px-4 font-bold text-blue-400">{host.ip}</td>
                      <td className="py-2.5 px-4 font-sans font-medium text-white truncate max-w-[180px]">
                        {host.hostname || <span className="text-slate-500 font-normal italic">Unresolved</span>}
                      </td>
                      <td className="py-2.5 px-4 text-slate-300">{host.macAddress}</td>
                      <td className="py-2.5 px-4 font-sans capitalize">
                      ...
                      <td className="py-2.5 px-4 text-slate-400 font-sans">
                        {host.openPorts.length > 0 ? (
                          host.openPorts.map((p) => (
                            <span
                              key={p}
                              className="inline-block px-1.5 py-0.2 mr-1 rounded bg-slate-800 text-[10px] text-slate-300 font-mono"
                            >
                              {p}
                            </span>
                          ))
                        ) : (
                          <span className="text-slate-500 text-[10px]">ICMP Echo</span>
                        )}
                      </td>
                      <td className="py-2.5 px-4 text-right font-sans font-medium">
                        {host.isExistingInIPAM ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] bg-blue-950 text-blue-300 border border-blue-800">
                            <CheckCircle2 className="w-3 h-3" />
                            <span>In IPAM</span>
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] bg-amber-950 text-amber-300 border border-amber-800">
        ...
        <div className="shrink-0 px-6 py-4 border-t border-slate-800 bg-slate-900 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="text-xs text-slate-400">
            {selectedCount > 0 ? (
              <span>
                <strong className="text-blue-400 font-semibold">{selectedCount} unmanaged host(s)</strong> selected for bulk allocation into {subnet.name}.
              </span>
            ) : (
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
            ...
            <button
              ...
              className="px-5 py-2 rounded-xl text-xs font-semibold bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:hover:bg-blue-600 text-white shadow-sm transition-colors flex items-center gap-1.5"
            >
