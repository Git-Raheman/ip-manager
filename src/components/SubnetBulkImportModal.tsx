...
export const SubnetBulkImportModal: React.FC<SubnetBulkImportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-3 sm:p-5">
      <div className="bg-white dark:bg-[#0c0c0c] border border-slate-200 dark:border-slate-800 rounded-2xl max-w-5xl w-full max-h-[92vh] flex flex-col shadow-2xl overflow-hidden text-slate-900 dark:text-slate-100">
        ...
        <div className="shrink-0 px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-[#0c0c0c]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center border border-emerald-500/20 shadow-inner">
              <FileSpreadsheet className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-slate-900 dark:text-white">Bulk Import IP Addresses</h3>
                <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200 dark:bg-blue-950 dark:text-blue-300 dark:border-blue-800 font-mono">
                  {subnet.cidr}
                </span>
              </div>
        ...
        <div className="shrink-0 px-6 py-3 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/70 dark:hover:bg-emerald-900/70 dark:text-emerald-300 dark:border-emerald-800 text-xs font-semibold transition-colors shadow-xs cursor-pointer"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 dark:border-slate-700 text-xs font-medium transition-colors cursor-pointer shadow-xs"
            >
        ...
        <div className="shrink-0 px-6 pt-3 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0c0c0c]">
          <div className="flex items-center gap-2">
            <button
              type="button"
          ...
          {parsedRows.length > 0 && (
            <button
              ...
              className="text-xs text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 flex items-center gap-1 pb-1 transition-colors cursor-pointer"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          {isParsing && (
            <div className="py-16 text-center space-y-3 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-2xl">
              <Loader2 className="w-8 h-8 text-blue-500 dark:text-blue-400 animate-spin mx-auto" />
              <p className="text-sm font-semibold text-slate-900 dark:text-white">Parsing and Validating IP Records...</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">Processing file rows with instant high-speed validation...</p>
            </div>
          )}
          ...
          {!isParsing && parsedRows.length === 0 && (
            activeTab === 'upload' ? (
              ...
              >
                <div className="w-14 h-14 rounded-2xl bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center justify-center border border-blue-500/20 mb-1">
                  <Upload className="w-7 h-7" />
                </div>
                <div>
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white">Drag &amp; drop your Excel or CSV file here</h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    Supports 50k+ records smoothly with instant in-memory processing
                  </p>
                </div>
                <span className="px-4 py-2 rounded-xl text-xs font-semibold bg-blue-600 hover:bg-blue-500 text-white shadow-xs transition-colors mt-2">
                  Browse Computer
                </span>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 font-mono text-xs text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:border-blue-500"
                />
                <button
                  type="button"
          ...
          {!isParsing && parsedRows.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-slate-600 dark:text-slate-400 font-medium">Analysis Summary:</span>
                  <span className="px-2 py-0.5 rounded bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300 font-semibold border border-blue-200 dark:border-blue-800">
                    Total: {parsedRows.length.toLocaleString()}
                  </span>
                  <span className="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300 font-semibold border border-emerald-200 dark:border-emerald-800">
                    Valid: {validCount.toLocaleString()}
                  </span>
                  {conflictCount > 0 && (
                    <span className="px-2 py-0.5 rounded bg-amber-50 text-amber-800 dark:bg-amber-950 dark:text-amber-300 font-semibold border border-amber-200 dark:border-amber-800">
                      Conflicts: {conflictCount.toLocaleString()}
                    </span>
                  )}
                  {invalidCount > 0 && (
                    <span className="px-2 py-0.5 rounded bg-rose-50 text-rose-700 dark:bg-rose-950 dark:text-rose-300 font-semibold border border-rose-200 dark:border-rose-800">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-slate-600 dark:text-slate-400 font-medium">When IP already exists:</span>
                  <select
                    value={conflictMode}
                    onChange={(e) => {
                    ...
                    className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg px-2.5 py-1 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 font-medium cursor-pointer shadow-xs"
                  >
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-2 px-1 text-xs text-slate-500 dark:text-slate-400">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    ...
                    of <strong className="text-slate-800 dark:text-slate-200">{parsedRows.length.toLocaleString()}</strong> parsed records.
                  </span>
                </div>
                ...
                {totalPages > 1 && (
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="p-1 rounded-lg bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-40 disabled:hover:bg-white dark:disabled:hover:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer shadow-xs"
                    >
                      <ChevronLeft className="w-4 h-4" />
                    </button>
                    <span className="px-2 text-slate-700 dark:text-slate-300 font-mono text-[11px]">
              ...
              <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-xs">
                <div className="overflow-x-auto max-h-[350px]">
                  <table className="w-full text-left border-collapse text-xs">
                    <thead className="sticky top-0 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 font-semibold z-10">
                      <tr>
                        ...
                        return (
                          <tr
                            ...
                            className={`transition-colors ${
                              !row.isValid
                                ? 'bg-rose-50/70 dark:bg-rose-950/20 text-rose-800 dark:text-rose-300'
                                : row.isConflict
                                ? 'bg-amber-50/70 dark:bg-amber-950/20 text-amber-800 dark:text-amber-200'
                                : row.isSelected
                                ? 'bg-blue-50/60 dark:bg-blue-950/30 text-slate-900 dark:text-slate-200'
                          ...
                          >
                            <td className="py-2.5 px-3 text-center">
                              {row.isValid ? (
                                ...
                                <AlertCircle className="w-4 h-4 text-rose-500 dark:text-rose-400 mx-auto" />
                              )}
                            </td>
                            <td className="py-2.5 px-3 font-bold font-mono text-blue-600 dark:text-blue-400">{row.ip}</td>
                            <td className="py-2.5 px-3 font-sans font-medium text-slate-900 dark:text-white truncate max-w-[140px]">
                              {row.hostname || <span className="text-slate-400 dark:text-slate-500 italic">None</span>}
                            </td>
                            <td className="py-2.5 px-3 text-slate-600 dark:text-slate-300">{row.macAddress || '-'}</td>
                            ...
                            <td className="py-2.5 px-3 font-sans text-slate-600 dark:text-slate-400 truncate max-w-[130px]">
                            ...
                            <td className="py-2.5 px-3 font-sans">
                              {!row.isValid ? (
                                ...
                                <span className="inline-flex items-center gap-1 text-amber-700 dark:text-amber-400 text-[11px] font-medium">
        ...
        {/* Modal Sticky Footer */}
        <div className="shrink-0 px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="text-xs text-slate-500 dark:text-slate-400">
            {parsedRows.length > 0 ? (
              <span>
                <strong className="text-blue-600 dark:text-blue-400 font-semibold">{selectedCount.toLocaleString()} IP record(s)</strong> selected for import.
              </span>
            ) : (
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
            ...
            <button
              ...
              className="px-5 py-2 rounded-xl text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:hover:bg-emerald-600 text-white shadow-xs transition-colors flex items-center gap-1.5 cursor-pointer active:scale-95"
            >
              {isImporting ? (
                <>
