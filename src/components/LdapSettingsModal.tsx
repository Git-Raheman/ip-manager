    ...
    <div className="space-y-6">
        ...
        {canManageAuth && (
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-[#171717] hover:bg-[#262626] dark:bg-slate-800 dark:hover:bg-slate-700 border border-[#ebebeb] dark:border-slate-700 text-white text-xs font-semibold transition-colors disabled:opacity-50"
            >
      ...
      {!canManageAuth && (
        <div className="p-4 rounded-2xl bg-amber-950/40 border border-amber-800/80 text-amber-200 text-xs flex items-center gap-3 shadow-sm">
          ...
          <div>
            <p className="font-semibold text-white">Read-Only Configuration Mode</p>
            <p className="text-[11px] text-amber-300/80 mt-0.5">
              Your account lacks the <code className="bg-amber-900/60 px-1 py-0.5 rounded text-amber-200 font-mono">manageAuthSettings</code> permission required to modify Active Directory / LDAP parameters.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm">
          <form onSubmit={handleSave} className="space-y-5">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div>
                <span className="font-semibold text-[#171717] dark:text-white text-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white placeholder-slate-600 font-mono focus:outline-none focus:border-blue-500"
                  />
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 font-mono text-xs shadow-inner">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800 text-slate-400">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-300">LDAP Diagnostic Stream</span>
              </div>
              {testResult && (
                <span
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm space-y-3">
            ...
            <div className="space-y-2 text-xs">
              <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between">
                <div>
                  <span className="font-semibold text-slate-200">Domain Admins</span>
              ...
              <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between">
                <div>
                  <span className="font-semibold text-slate-200">Network Engineers</span>
              ...
              <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between">
                <div>
                  <span className="font-semibold text-slate-200">NOC Operators</span>
              ...
              <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between">
                <div>
                  <span className="font-semibold text-slate-200">Compliance Auditors</span>
                  <p className="text-[10px] text-slate-500 font-mono">CN=IPAM-Auditors,OU=Groups</p>
                </div>
                <span className="px-2 py-0.5 rounded bg-purple-950 text-purple-300 border border-purple-800 text-[10px] font-medium">
                  Auditor
                </span>
              </div>
            </div>
            ...
            {registeredAdUsers.length === 0 ? (
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {registeredAdUsers.map((u) => (
                  ...
                  >
                    <div>
                      <div className="font-semibold text-[#171717] dark:text-white">{u.fullName || u.username}</div>
                      <div className="text-[10px] text-slate-400 font-mono">{u.ldapUpn || u.email || u.username}</div>
                    </div>
                    <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700 text-[10px] capitalize">
                      {u.role.replace('_', ' ')}
                    </span>
                  </div>
                ))}
