    ...
    try {
      ...
      const normalizedData: IPAMBackupData = {
        ...
        metadata: parsed.metadata || {
          ...
          totalSubnets: parsed.data?.subnets?.length || parsed.subnets?.length || 0,
          totalIPs: parsed.data?.ips?.length || parsed.ips?.length || 0,
          totalUsers: parsed.data?.users?.length || parsed.users?.length || 0,
          totalDeviceClassifications: parsed.data?.deviceClassifications?.length || 0,
          totalAuditLogs: parsed.data?.auditLogs?.length || 0,
          ldapConfigured: !!(parsed.data?.ldapConfig?.enabled || parsed.ldapConfig?.enabled),
        },
        data: {
          subnets: parsed.data?.subnets || parsed.subnets || [],
          ips: parsed.data?.ips || parsed.ips || [],
          users: parsed.data?.users || parsed.users || [],
          ldapConfig: parsed.data?.ldapConfig || parsed.ldapConfig || ldapConfig,
          deviceClassifications: parsed.data?.deviceClassifications || parsed.deviceClassifications || [],
          auditLogs: parsed.data?.auditLogs || parsed.auditLogs || [],
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-cyan-600 to-blue-600 text-white flex items-center justify-center shadow-lg shadow-cyan-900/30">
                <Database className="w-6 h-6" />
              </div>
              <div>
                <h1 className="text-2xl font-bold text-[#171717] dark:text-white tracking-tight">System Backup &amp; Disaster Recovery</h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-3 text-center">
              <span className="text-[11px] text-slate-400 font-medium block">Subnets</span>
              <span className="text-lg font-bold text-cyan-400">{subnets.length}</span>
            </div>
            <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-3 text-center">
              <span className="text-[11px] text-slate-400 font-medium block">Allocated IPs</span>
              <span className="text-lg font-bold text-blue-400">{ips.length}</span>
            </div>
            <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-3 text-center">
              <span className="text-[11px] text-slate-400 font-medium block">User Accounts</span>
              <span className="text-lg font-bold text-indigo-400">{users.length}</span>
            </div>
            <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-3 text-center">
              <span className="text-[11px] text-slate-400 font-medium block">AD / LDAP</span>
              <span className={`text-xs font-bold inline-flex items-center gap-1 mt-1 ${ldapConfig.enabled ? 'text-emerald-400' : 'text-slate-500'}`}>
      ...
      {restoreSuccessResult && (
        <div className="bg-emerald-950/40 border border-emerald-800/80 rounded-2xl p-4 flex items-start justify-between gap-3 text-emerald-200 animate-in fade-in">
          <div className="flex items-start gap-3">
            <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" />
            <div>
              <h4 className="text-sm font-semibold text-white">System Restored Successfully</h4>
              <p className="text-xs text-emerald-300/90 mt-0.5">{restoreSuccessResult}</p>
            </div>
          </div>
          <button
            onClick={() => setRestoreSuccessResult(null)}
            className="text-xs text-emerald-400 hover:text-emerald-200 font-medium underline"
          >
            Dismiss
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* ========================================================= */}
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-7 shadow-sm flex flex-col justify-between space-y-6">
          <div className="space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-[#171717] dark:text-white">Export Full Backup</h2>
                  <p className="text-xs text-slate-400">Download complete system state as a verifiable JSON package</p>
                </div>
              </div>
              <span className="text-[11px] px-2.5 py-1 rounded-full bg-blue-950 text-blue-300 border border-blue-800 font-mono">
                JSON v1.4
              </span>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                <div className="flex items-center gap-2.5 bg-slate-950/60 border border-slate-800/80 rounded-xl px-3.5 py-2.5 text-xs text-slate-200">
                  <Network className="w-4 h-4 text-cyan-400 shrink-0" />
                  <div className="truncate">
                    <span className="font-semibold">{subnets.length}</span> Subnets &amp; VLANs
                  </div>
                </div>
                <div className="flex items-center gap-2.5 bg-slate-950/60 border border-slate-800/80 rounded-xl px-3.5 py-2.5 text-xs text-slate-200">
                  <Sliders className="w-4 h-4 text-blue-400 shrink-0" />
                  <div className="truncate">
                    <span className="font-semibold">{ips.length}</span> IP Allocations
                  </div>
                </div>
                <div className="flex items-center gap-2.5 bg-slate-950/60 border border-slate-800/80 rounded-xl px-3.5 py-2.5 text-xs text-slate-200">
                  <Users className="w-4 h-4 text-indigo-400 shrink-0" />
                  <div className="truncate">
                    <span className="font-semibold">{users.length}</span> User Accounts &amp; Roles
                  </div>
                </div>
                <div className="flex items-center gap-2.5 bg-slate-950/60 border border-slate-800/80 rounded-xl px-3.5 py-2.5 text-xs text-slate-200">
                ...
                <div className="flex items-center gap-2.5 bg-slate-950/60 border border-slate-800/80 rounded-xl px-3.5 py-2.5 text-xs text-slate-200">
                  ...
                  <div className="truncate">
                    <span className="font-semibold">{deviceClassifications.length}</span> Device Classes
                  </div>
                </div>
                <div className="flex items-center gap-2.5 bg-slate-950/60 border border-slate-800/80 rounded-xl px-3.5 py-2.5 text-xs text-slate-200">
                  <FileText className="w-4 h-4 text-emerald-400 shrink-0" />
                  <div className="truncate">
                    <span className="font-semibold">{auditLogs.length}</span> Security Audit Logs
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-3.5 space-y-2">
              <label className="flex items-center justify-between text-xs text-slate-300 cursor-pointer select-none">
                <span className="flex items-center gap-2">
                  ...
                  <span>Include Security Audit Trail in backup</span>
                </span>
                <input
                  type="checkbox"
                  checked={includeAuditInExport}
                  onChange={(e) => setIncludeAuditInExport(e.target.checked)}
                  className="w-4 h-4 rounded text-blue-600 bg-slate-900 border-slate-700 focus:ring-blue-500 focus:ring-offset-slate-950"
                />
              </label>
            ...
            <div className="bg-slate-950/40 border border-slate-800/60 rounded-xl p-3.5 text-xs text-slate-400 space-y-1 font-mono">
              <div className="flex justify-between">
                <span>Backup Format:</span>
                <span className="text-slate-300">JSON (Universal IPAM Snapshot)</span>
              </div>
              <div className="flex justify-between">
                <span>Generated By:</span>
                <span className="text-slate-300">@{currentUser?.username || 'system'}</span>
              </div>
              <div className="flex justify-between">
                <span>Timestamp:</span>
            ...
            <div>
              ...
              {showExportPreview && (
                <div className="mt-3 relative">
                  <div className="max-h-56 overflow-auto bg-slate-950 border border-slate-800 rounded-xl p-3 text-[11px] font-mono text-slate-300">
                    <pre>{JSON.stringify(liveBackup, null, 2)}</pre>
                  </div>
                  <button
                    onClick={handleCopyBackup}
                    className="absolute top-2.5 right-2.5 px-2.5 py-1 rounded bg-slate-800/90 hover:bg-slate-700 text-[10px] font-medium text-slate-200 border border-slate-700 flex items-center gap-1 shadow-sm transition-colors"
                  >
                    {copiedBackup ? (
                      <>
          ...
          <div className="space-y-3 pt-4 border-t border-slate-800">
            <button
              ...
              className={`w-full flex items-center justify-center gap-2.5 px-5 py-3.5 rounded-xl font-semibold text-sm shadow-md transition-all ${
                canExport
                  ? 'bg-blue-600 hover:bg-blue-500 text-white cursor-pointer active:scale-[0.99]'
                  : 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700'
              }`}
            >
              {canExport ? <Download className="w-4 h-4" /> : <Lock className="w-4 h-4 text-slate-500" />}
            ...
            {canExport && (
              <button
                onClick={handleCopyBackup}
                className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-slate-800/80 hover:bg-slate-800 text-slate-300 hover:text-white text-xs font-medium border border-slate-700 transition-colors"
              >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-7 shadow-sm flex flex-col justify-between space-y-6">
          <div className="space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-[#171717] dark:text-white">Restore Everything</h2>
                  <p className="text-xs text-slate-400">Restore subnets, IPs, users, and settings from a backup file</p>
                </div>
              </div>
              <span className="text-[11px] px-2.5 py-1 rounded-full bg-emerald-950 text-emerald-300 border border-emerald-800 font-mono">
                Disaster Recovery
              </span>
            </div>
            ...
            <div className="flex items-center gap-1 bg-slate-950/60 p-1 rounded-xl border border-slate-800/80">
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-medium rounded-lg transition-colors flex items-center justify-center gap-1.5 ${
                  activeImportMode === 'file' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-medium rounded-lg transition-colors flex items-center justify-center gap-1.5 ${
                  activeImportMode === 'paste' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
                <FileJson className="w-3.5 h-3.5" />
            ...
            {activeImportMode === 'file' && (
              <div className="space-y-2">
                ...
                <label
                  ...
                  className="border-2 border-dashed border-slate-700 hover:border-blue-500/60 bg-slate-950/40 hover:bg-slate-950/80 rounded-2xl p-6 flex flex-col items-center justify-center text-center cursor-pointer transition-all group"
                >
                  <div className="w-12 h-12 rounded-xl bg-slate-800 group-hover:bg-blue-600/20 text-slate-400 group-hover:text-blue-400 flex items-center justify-center mb-3 transition-colors">
                    <FileJson className="w-6 h-6" />
                  </div>
                  <span className="text-sm font-semibold text-slate-200 group-hover:text-[#171717] dark:group-hover:text-white">
            ...
            {activeImportMode === 'paste' && (
              <div className="space-y-2">
                <textarea
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs font-mono text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none placeholder:text-slate-600"
                />
                <button
                  type="button"
            ...
            {validationError && (
              <div className="bg-rose-950/40 border border-rose-800/80 rounded-xl p-3 text-xs text-rose-300 flex items-center gap-2.5">
            ...
            {uploadedBackup && (
              <div className="bg-slate-950/70 border border-emerald-500/30 rounded-xl p-4 space-y-4 animate-in fade-in">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-2 text-center">
                    <span className="text-[10px] text-slate-400 block">Subnets</span>
                    <span className="text-sm font-bold text-cyan-400">
                      {uploadedBackup.data.subnets?.length || 0}
                    </span>
                  </div>
                  <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-2 text-center">
                    <span className="text-[10px] text-slate-400 block">IP Records</span>
                  ...
                  <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-2 text-center">
                    <span className="text-[10px] text-slate-400 block">Device Classes</span>
                    <span className="text-sm font-bold text-purple-400">
                      {uploadedBackup.data.deviceClassifications?.length || 0}
                    </span>
                  </div>
                  <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-2 text-center">
                    <span className="text-[10px] text-slate-400 block">Audit Entries</span>
                    <span className="text-sm font-bold text-emerald-400">
                      {uploadedBackup.data.auditLogs?.length || 0}
                    </span>
                  </div>
                  <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-2 text-center">
                    <span className="text-[10px] text-slate-400 block">LDAP Status</span>
                ...
                <div className="pt-2 border-t border-slate-800/80 space-y-2.5">
                  ...
                  {/* Safety Guard Option */}
                  <label className="flex items-center gap-2 text-xs text-amber-300/90 pt-1 cursor-pointer">
                    <input
                      type="checkbox"
          ...
          <div className="space-y-3 pt-4 border-t border-slate-800">
            {!isSuperAdmin && (
              <div className="p-3.5 rounded-xl bg-amber-950/40 border border-amber-800/80 text-amber-200 text-xs flex items-center gap-2.5">
            ...
            >
              {isSuperAdmin ? <RotateCcw className="w-4 h-4" /> : <Lock className="w-4 h-4 text-slate-500" />}
              <span>
                {!isSuperAdmin
      ...
      {confirmRestoreModalOpen && uploadedBackup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 sm:p-7 shadow-2xl text-slate-100 space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-[#171717] dark:text-white">Confirm System Restore</h3>
                <p className="text-xs text-slate-400">
            ...
            <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 text-xs space-y-2 text-slate-300">
              <p className="font-semibold text-[#171717] dark:text-white">The following datasets will be applied:</p>
              <ul className="list-disc list-inside space-y-1 text-slate-400">
                {restoreOptions.restoreSubnets && (
                  <li>
                    <strong className="text-slate-200">{uploadedBackup.data.subnets?.length || 0} Subnets</strong>{' '}
                    ({restoreOptions.mode === 'overwrite' ? 'replacing existing subnets' : 'merging with current'})
                  </li>
                )}
                {restoreOptions.restoreIPs && (
                  <li>
                    <strong className="text-slate-200">{uploadedBackup.data.ips?.length || 0} IP Allocations</strong>
                  </li>
                )}
                {restoreOptions.restoreUsers && (
                  <li>
                    <strong className="text-slate-200">{uploadedBackup.data.users?.length || 0} User Accounts</strong>{' '}
                    and assigned permission matrices
                  </li>
                )}
                {restoreOptions.restoreLdap && (
                  <li>
                    <strong className="text-slate-200">Active Directory / LDAP Config</strong>{' '}
              ...
              {createSafetyBackupFirst && (
                <div className="pt-2 text-amber-400 font-medium flex items-center gap-1.5">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white shadow-md transition-colors"
              >
      ...
      {resetModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl text-slate-100">
            <h3 className="text-lg font-semibold text-[#171717] dark:text-white mb-2">Reset to Default System State?</h3>
