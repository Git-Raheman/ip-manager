...
app.post('/api/ips', authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'INSERT INTO ips (ip, hostname, ports, status, note, tab_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [ip, hostname || '', ports || '', status || 'active', note || '', tab_id || null]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
    console.error(err);
...
app.put('/api/ips/:id', authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'UPDATE ips SET ip = COALESCE($1, ip), hostname = COALESCE($2, hostname), ports = COALESCE($3, ports), status = COALESCE($4, status), note = COALESCE($5, note), tab_id = COALESCE($6, tab_id) WHERE id = $7 RETURNING *',
      [ip, hostname, ports, status, note, tab_id, id]
    );
