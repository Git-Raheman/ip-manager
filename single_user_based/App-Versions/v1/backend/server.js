...
// Auth Middleware (Simplified for Single User)
...
// Seed Admin User (Single User)
const seedAdmin = async () => {
  try {
    const res = await pool.query('SELECT * FROM users WHERE username = $1', ['admin']);
    if (res.rows.length === 0) {
      const hash = await bcrypt.hash('admin123', 10);
      // We keep 'role' column interaction to satisfy existing DB schema constraint
...
app.put('/api/tabs/:id/share', authenticateToken, async (req, res) => {
  const { id } = req.params;
  const { enable } = req.body; // true = generate token, false = remove

  try {
    let token = null;
    if (enable) {
      // Simple UUID-like generation or use crypto
      const crypto = require('crypto');
      token = crypto.randomUUID();
    }

    const result = await pool.query(
      'UPDATE tabs SET share_token = $1 WHERE id = $2 RETURNING *',
      [token, id]
    );

    if (result.rows.length === 0) return res.status(404).json({ error: 'Tab not found' });

    logSystemEvent('TAB', `Updated share status for tab ${result.rows[0].name} (Public: ${enable})`, req.user.id);
...
app.get('/api/public/view/:token', async (req, res) => {
  ...
  console.log(`[PublicView] Access attempt with token: "${token}"`);

  if (!token || token.length < 10) {
    console.log('[PublicView] Invalid token length or missing');
    return res.status(404).json({ error: 'Invalid token' });
  }

  try {
    // 1. Verify Token & Get Tab
    const tabRes = await pool.query('SELECT * FROM tabs WHERE share_token = $1', [token]);
    if (tabRes.rows.length === 0) {
      console.log(`[PublicView] Token not found in DB: "${token}"`);
      return res.status(404).json({ error: 'Shared view not found or disabled' });
    }

    const tab = tabRes.rows[0];
    console.log(`[PublicView] Found tab: ${tab.name} (ID: ${tab.id})`);

    // 2. Fetch IPs for this tab
    let query = 'SELECT ip, hostname, ports, status, note, last_status, last_checked FROM ips WHERE tab_id = $1';
    const params = [tab.id];

    if (search) {
      query += ` AND (ip ILIKE $2 OR hostname ILIKE $2 OR status ILIKE $2 OR note ILIKE $2)`;
      params.push(`%${search}%`);
    }

    // Simplified sort to avoid potential syntax errors during regex parsing
    query += " ORDER BY ip ASC LIMIT 500";

    console.log(`[PublicView] Executing Query: ${query}`);
    console.log(`[PublicView] Params: ${JSON.stringify(params)}`);

    const ipRes = await pool.query(query, params);
    console.log(`[PublicView] Returned ${ipRes.rows.length} IPs`);
    ...
    console.error('[PublicView] Error:', err);
    res.status(500).json({ error: 'Server error' });
  }
});
...
app.post('/api/tabs', authenticateToken, async (req, res) => {
  const { name } = req.body;
...
app.delete('/api/tabs/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;
  try {
...
app.post('/api/ips/bulk-delete', authenticateToken, async (req, res) => {
  const { ids } = req.body;
...
// Excel/CSV Import
app.post('/api/backup/import-excel', authenticateToken, upload.single('file'), async (req, res) => {
...
app.post('/api/monitor/config', authenticateToken, async (req, res) => {
...
app.post('/api/monitor/ping-all', authenticateToken, async (req, res) => {
  runPingJob();
...
app.post('/api/monitor/ping/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;
  try {
...
const initDB = async () => {
  ...
  try {
    ...
    await client.query(`ALTER TABLE tabs ADD COLUMN IF NOT EXISTS share_token VARCHAR(64) DEFAULT NULL`);
