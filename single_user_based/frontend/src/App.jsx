...
import {
    ...
    Plus, List, LayoutGrid, Network, Search, Trash2, Download, Edit2, X, Share2, Globe, Upload
} from 'lucide-react';
...
export default function App() {
    ...
    // Public Mode State
    const [publicMode, setPublicMode] = useState({ active: false, token: null, tabName: '' });
    ...
    // UI State
    const [view, setView] = useState('ips'); // ips, monitor
    ...
    const [backupModalTab, setBackupModalTab] = useState('json');
    ...
    // Initial Load: Check for Public Share Token OR Auth
    useEffect(() => {
        const params = new URLSearchParams(window.location.search);
        const shareToken = params.get('share');

        if (shareToken) {
            setPublicMode({ active: true, token: shareToken, tabName: 'Loading...' });
            fetchPublicIps(shareToken);
        } else {
            setLoading(false); // Let AuthContext handle the rest
        }
    }, []);

    // Fetch Private Data (Only if logged in and not public)
    useEffect(() => {
        if (!publicMode.active && user) {
            fetchTabs();
            fetchMonitorStatus();
            setLoading(false);
        }
    }, [user, publicMode.active]);

    // Fetch IPs (Private or Refetch)
    useEffect(() => {
        if (!publicMode.active && user && (view === 'ips')) {
            fetchIps();
        }
    }, [user, pagination.currentPage, debouncedSearch, activeTab, view, sortConfig, itemsPerPage, publicMode.active]);

    const fetchPublicIps = async (token) => {
        setLoading(true);
        try {
            const queryParams = new URLSearchParams({
                search: debouncedSearch
            });
            const res = await fetch(`/api/public/view/${token}?${queryParams}`);
            if (res.ok) {
                const data = await res.json();
                setAllIps(data.ips || []);
                setPublicMode(prev => ({ ...prev, active: true, token, tabName: data.tabName }));
                setPagination({ currentPage: 1, totalPages: 1, totalItems: data.ips.length });
            } else {
                alert('Shared tab not found or disabled.');
                setPublicMode({ active: false, token: null, tabName: '' });
                window.history.replaceState({}, '', '/');
            }
        } catch (e) { console.error("Failed to fetch public IPs", e); }
        setLoading(false);
    };
    ...
    const fetchTabs = async () => {
        try {
            ...
            if (res.ok) {
                ...
                // Single User - No allowed_tabs filtering needed
    ...
    const handleShareTab = async (id, existingToken) => {
        const token = localStorage.getItem('token');
        const tab = tabs.find(t => t.id === id);
        const isShared = !!tab.share_token;

        if (isShared) {
            const link = `${window.location.origin}/?share=${tab.share_token}`;
            if (window.confirm(`Tab is currently SHARED.\nLink: ${link}\n\nDo you want to DISABLE sharing for this tab?`)) {
                updateShare(id, false);
            } else {
                navigator.clipboard.writeText(link);
                alert('Link copied to clipboard!');
            }
        } else {
            if (window.confirm('Enable public read-only link for this tab?')) {
                updateShare(id, true);
            }
        }
    };

    const updateShare = async (id, enable) => {
        const token = localStorage.getItem('token');
        const res = await fetch(`/api/tabs/${id}/share`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        ...
        if (res.ok) {
            fetchTabs();
            if (enable) alert('Sharing Enabled. Click the share icon again to get the link.');
    ...
    const handleExportExcel = async () => {
        if (publicMode.active) {
            alert("Export not available in public view.");
            return;
        }
        const token = localStorage.getItem('token');
    ...
    if (loading || authLoading) return <div className="loading-screen"><div className="spinner"></div></div>;

    // Auth Check
    if (!publicMode.active && !user) return <Login />;
    ...
    return (
        <div className="container fade-in">
            <header>
                <div className="header-top glass-effect">
                    <h1>
                        ...
                        {publicMode.active ? ` Shared View: ${publicMode.tabName}` : ' IP Manager'}
                    </h1>
                    ...
                    {!publicMode.active && (
                        <div className="user-controls">
                            ...
                            <button onClick={() => { setBackupModalTab('json'); setShowBackupModal(true); }} className="btn-backup" title="Backup & Restore">
                                <Save size={16} /> Backup
                            </button>
                    ...
                    {publicMode.active && (
                        <div className="user-controls">
                            <a href="/" className="btn-secondary" style={{ textDecoration: 'none' }}>Login</a>
                        </div>
                    )}
                </div>
            </header>
            ...
            <main>
                {/* Monitor View */}
                {view === 'monitor' && !publicMode.active && (
                    <MonitorView
                        ...
                        onPingOne={async (ip) => {
                            ...
                            if (res.ok) {
                                ...
                                fetchIps(); // Refresh IPs to show status
                            } else {
                    ...
                    <>
                        ...
                        <section className="card list-card">
                            <div className="list-header">
                                ...
                                {selectedIds.length > 0 && !publicMode.active && (
                                ...
                                {!publicMode.active && (
                                    <div className="action-buttons-group">
                                        <button onClick={() => { setBackupModalTab('excel'); setShowBackupModal(true); }} className="btn-secondary" title="Import IPs from Excel">
                            ...
                            {/* Tabs - Only show in Admin mode (which is always if not public) */}
                            {!publicMode.active && (
                                            ...
                                            <div key={tab.id} className={`tab ${activeTab === tab.id ? 'active' : ''}`} onClick={() => setActiveTab(tab.id)}>
                                                ...
                                                {tab.id !== 'all' && (
                                                    <div className="tab-actions">
                                                        <button
                                                            className={`btn-tab-share ${tab.share_token ? 'shared' : ''}`}
                                                            onClick={(e) => { e.stopPropagation(); handleShareTab(tab.id, tab.share_token); }}
                                                            title={tab.share_token ? "Shared (Click to copy/disable)" : "Enable Public Share"}
                                                        >
                                                            {tab.share_token ? <Globe size={12} /> : <Share2 size={12} />}
                                                        </button>
                            ...
                            {/* MAIN VIEWS */}
                            {displayMode === 'pool' && <PoolView ips={allIps} onEdit={!publicMode.active ? setEditingIp : undefined} />}
                                        ...
                                        <table className="ip-table">
                                            <thead>
                                                <tr>
                                                    ...
                                                    {activeTab === 'all' && !publicMode.active && <th>Tab</th>}
                                                    ...
                                                    {!publicMode.active && <th>Actions</th>}
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {allIps.map(ip => (
                                                    <tr key={ip.id} className={selectedIds.includes(ip.id) ? 'selected-row' : ''}>
                                                        ...
                                                        <td>
                                                            {!publicMode.active ? (
                                                                ...
                                                                <span className={`status-badge ${ip.status}`}>{ip.status}</span>
                                                            )}
                                                        </td>
                                                        {activeTab === 'all' && !publicMode.active && (
                                                            <td>
            ...
            {/* Modals - Only show relevant ones */}
            <ChangePasswordModal
            ...
            />
            <BackupModal isOpen={showBackupModal} onClose={() => setShowBackupModal(false)} initialTab={backupModalTab} />
