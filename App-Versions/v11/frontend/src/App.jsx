...
const LogsTab = () => {
    ...
    const [autoDeleteConfig, setAutoDeleteConfig] = useState({ enabled: false, hours: 24 });
    ...
    const handleSaveAutoDelete = async () => {
        ...
        const res = await fetch('/api/logs/auto-delete-config', {
            ...
            body: JSON.stringify({ enabled: autoDeleteConfig.enabled, hours: configHours })
        });
        ...
        <div className="card logs-card">
            ...
            {showConfig && (
                <div className="logs-config-panel">
                    <h4>Auto-Delete Configuration</h4>
...
const PingStatusModal = ({ isOpen, onClose, ipData, onRefresh }) => {
    ...
    // Auto-close after 5 seconds when result is available
    useEffect(() => {
        if (result && !pinging) {
            ...
            <div className="ping-toast-body">
                {pinging ? (
                    ...
                    result.error ? (
                        ...
                        <div className="ping-result-small success">
                            {result.status === 'UP' ? (
                                <CheckCircle size={20} color={getStatusColor(result.status)} />
                            ) : (
                                <XCircle size={20} color={getStatusColor(result.status)} />
                            )}
                            <span style={{ color: getStatusColor(result.status) }}>
                                {result.status === 'UP' ? 'Host is UP' :
                                    result.status === 'RESERVED' ? 'Host is RESERVED' : 'Host is DOWN'}
                            </span>
                        </div>
                    )
                ) : null}
