...
app.put('/api/users/:id/password', authenticateToken, requireSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    logSystemEvent('USER_MGMT', `Updated password for user ID ${id}`, req.user.id);
...
app.post('/api/backup/import', authenticateToken, requireAdmin, async (req, res) => {
  ...
  // For brevity, assuming existing JSON import logic is fine.
  // I will include it to be safe.
  const { data, mode } = req.body;
  if (!data || !['merge', 'replace'].includes(mode)) return res.status(400).json({ error: 'Invalid data' });
  ...
  try {
    ...
    // Skipping detailed re-implementation of JSON import for brevity in this thought trace, 
    // but in the actual file write I MUST include it.
    // I'll copy the logic from previous version.
    ...
    if (data.tabs) {
      for (const tab of data.tabs) {
        await client.query(
          `INSERT INTO tabs (id, name, created_at) VALUES ($1, $2, $3) ON CONFLICT (name) DO NOTHING`,
      ...
      for (const ip of data.ips) {
        await client.query(
          `INSERT INTO ips (ip, hostname, ports, status, note, tab_id, created_by, last_updated_by, subnet, cidr, last_checked, last_status, created_at)
                     ...
                     ON CONFLICT (ip, subnet, cidr) DO UPDATE SET hostname=EXCLUDED.hostname, ports=EXCLUDED.ports, status=EXCLUDED.status, note=EXCLUDED.note, tab_id=EXCLUDED.tab_id`,
          [ip.ip, ip.hostname, ip.ports, ip.status, ip.note, ip.tab_id, ip.created_by, ip.last_updated_by, ip.subnet || '', ip.cidr || '', ip.last_checked, ip.last_status, ip.created_at]
        );
...
// Clear all logs
app.delete('/api/logs/clear', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
...
// Export logs as JSON
app.get('/api/logs/export', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
...
app.get('/api/logs/auto-delete-config', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
...
app.post('/api/logs/auto-delete-config', authenticateToken, requireSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    logSystemEvent('LOGS', `Updated auto-delete config: ${enabled ? 'ON' : 'OFF'}, ${hours} hours`, req.user.id);
    res.json({ message: 'Auto-delete configuration updated', enabled, hours });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Database error' });
  }
...
app.post('/api/logs/auto-delete-run', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    const hoursRes = await pool.query("SELECT value FROM settings WHERE key = 'logs_auto_delete_hours'");
    const hours = hoursRes.rows.length > 0 ? parseInt(hoursRes.rows[0].value) : 24;
...
app.post('/api/monitor/ping-one', authenticateToken, requireSuperAdmin, async (req, res) => {
  const { ip } = req.body;
  ...
  try {
    ...
    await pool.query('UPDATE ips SET last_status = $1, last_checked = NOW() WHERE id = $2', [newStatus, id]);
    res.json({ message: 'Ping check completed', status: newStatus });
  } catch (err) {
    console.error(err);
