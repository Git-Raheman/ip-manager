 ...
 * Handles system backup/restore via JSON and Excel import.
 */
const BackupModal = ({ isOpen, onClose, initialTab = 'json' }) => {
    ...
    const handleImportJson = async () => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                setMsg('Import failed: Invalid JSON or server error');
            }
        };
            ...
            <div className="modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-body">
                    <div className="backup-tabs">
                        <button className={`tab-btn ${tab === 'json' ? 'active' : ''}`} onClick={() => setTab('json')}>JSON Backup</button>
                        <button className={`tab-btn ${tab === 'excel' ? 'active' : ''}`} onClick={() => setTab('excel')}>Excel Import</button>
                    </div>
                            ...
                            <div className="action-row">
                                <div className="import-controls">
                                    <select
                                        ...
                                        className="mode-select modern-select"
                                    >
                                        <option value="replace">Replace (Overwrite All)</option>
                                        <option value="merge">Merge (Keep Existing)</option>
                                    </select>
                                    <input type="file" accept=".json" onChange={e => setFile(e.target.files[0])} className="file-input" />
                    ...
                    {tab === 'excel' && (
                        <div className="backup-section">
                            ...
                            <div className="action-row">
                                <input type="file" accept=".xlsx, .xls" onChange={e => setExcelFile(e.target.files[0])} className="file-input" />
